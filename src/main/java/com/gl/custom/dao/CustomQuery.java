...
public class CustomQuery {
    ...
    public static void saveInGdceauthtoken(Connection conn, String token, String secs) {
        deleteFromGdceAuthToken(conn);
        String query = "insert into gdce_auth_token_detail (token , expire_on) values ( '" + token + "' , DATE_ADD(now(), INTERVAL " + secs + " - 60 second) ) ";
        logger.info("Query " + query);
        try (Statement st = conn.createStatement()) {
    ...
    public static String checkInGdceData(Connection conn, String imei) {
        ...
        logger.info("Gdce data Response :" + response);
        return response;
    }
    ...
    public static void saveInGdceData(Connection conn, String imei, Result r, String source, String reqId) {
        String query = "insert into app.gdce_data(imei,serial_number,registration_date,date_of_actual_import,is_custom_tax_paid, actual_imei, source,request_id ,sim,brand,model,device_type,goods_description   ) " +
                "values( '" + imei.substring(0, 14) + "', '" + r.getSerial_number() + "', '" + r.getDate_of_registration() + "', '" + r.getDate_of_actual_import() + "','" + (r.getCustoms_duty_tax().equalsIgnoreCase("paid") ? 1 : 0) + "' , '" + imei + "', '" + source + "', '" + reqId + "' , '" + r.getSim() + "' , '" + r.getBrand() + "' , '" + r.getModel() + "' , '" + r.getDevice_type() + "' , '" + r.getGoods_description() + "'  )";  //r.
        logger.debug("Query " + query);
        try (Statement st = conn.createStatement()) {
